...
def transform(df_purchase, df_receipt):
    # Merge purchase id
    df_purchase.rename(columns={"id":"purchase_id"},inplace=True)
    df_receipt.rename(columns={"item_code":"code","item_name":"name","item_quantity":"quantity","item_price":"price","item_is_discount":"is_discount"},inplace=True)
    df = df_receipt.merge(df_purchase, on=["photo"], how="left")
    
    # Get needed columns
    columns = ["code","name","quantity","price","is_discount","purchase_id"]
    df = df[columns].copy()
...
def get_loads(df_old, df_new) -> None:
    ...
    # Create a composite key
    df_old["key"] = df_old["code"].astype("string") + "_" +df_old["purchase_id"].astype("string")
    df_new["key"] = df_new["code"].astype("string") + "_" +df_new["purchase_id"].astype("string")
...
@log_msg(message="Insert personal item", slack_log=True, add_breakline=True)
def insert_item():
    # Create db client
    ...
    old_item_cols,old_item_rows = finance_db.execute_query(query=old_item_query)
    df_old_item = pd.DataFrame(data=old_item_rows,columns=old_item_cols)
    ...
    # Transform
    df_new_item = transform(df_old_purchase, df_old_receipt)

    # Load
    inserts, updates, deletes = get_loads(df_old_item,df_new_item)
    with finance_session.begin():
        finance_session.bulk_insert_mappings(PersonalItem, inserts)
        finance_session.bulk_update_mappings(PersonalItem, updates)
        finance_session.query(PersonalItem).filter(PersonalItem.id.in_([row["id"] for row in deletes])).delete()
